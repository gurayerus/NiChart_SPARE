...
class TestOutputFormat:
    def test_cl_col0_is_mrid(self, raw_cl_csv, tmp_path):
        df, _ = prep_data(raw_cl_csv, key_col='MRID', target_col='DX',
                          data_cols=['DL_MUSE_Volume_*'],
    ...
    def test_unlisted_columns_excluded(self, raw_cl_csv, tmp_path):
        # 'Study' is not in data_cols so should be absent from output
        df, _ = prep_data(raw_cl_csv, key_col='MRID', target_col='DX',
                          data_cols=['DL_MUSE_Volume_*'],
                          output_file=str(tmp_path / 'out.csv'))
    ...
    def test_rg_col1_is_target(self, raw_rg_csv, tmp_path):
        df, _ = prep_data(raw_rg_csv, key_col='MRID', target_col='Age',
                          data_cols=['DL_MUSE_Volume_*'],
                          output_file=str(tmp_path / 'out.csv'))
...
class TestInferenceMode:
    """When target_col is absent or not in the input, output should have no target column."""

    def test_no_target_col_in_output(self, raw_cl_csv, tmp_path):
        # data_cols doesn't include DX, so it will be absent regardless
        df, _ = prep_data(raw_cl_csv, key_col='MRID', target_col=None,
                          data_cols=['DL_MUSE_Volume_*'],
                          output_file=str(tmp_path / 'out.csv'))
...
class TestMappings:
    def test_mapping_applied(self, tmp_path):
        import numpy as np
        df = pd.DataFrame({
            'MRID': [f'S{i}' for i in range(10)],
            'DX':   np.random.randint(0, 2, 10),
            'Sex':  ['M', 'F'] * 5,
            'DL_MUSE_Volume_1': np.random.normal(5_000, 500, 10),
        })
        df.to_csv(tmp_path / 'mapped.csv', index=False)
        out, _ = prep_data(str(tmp_path / 'mapped.csv'),
                           key_col='MRID', target_col='DX',
                           data_cols=['Sex', 'DL_MUSE_Volume_*'],
                           mappings={'Sex': {'M': 0, 'F': 1}})
        assert set(out['Sex'].unique()).issubset({0, 1})


class TestCVMPrep:
    def test_confounds_removed_from_features(self, raw_cvm_csv, tmp_path):
        df, _ = prep_data(raw_cvm_csv, key_col='MRID', target_col='Disease',
    ...
    def test_cvm_missing_confound_raises(self, tmp_path):
        """Prep should raise if required residualization columns are absent."""
        import numpy as np
        bad = pd.DataFrame({
            'MRID': [f'S{i}' for i in range(10)],
        ...
        with pytest.raises(ValueError, match='residualization requires'):
            prep_data(str(tmp_path / 'bad.csv'), key_col='MRID',
                      target_col='Disease',
                      data_cols=['H_DL_MUSE_Volume_*'],
                      preprocessing={'residualization': {
                          'age_col': 'Age', 'sex_col': 'Sex',
                          'icv_col': 'DL_MUSE_Volume_702',
                      }})
...
class TestICVCorrection:
    def test_icv_correction_runs(self, tmp_path):
        ...
        out, _ = prep_data(str(tmp_path / 'icv.csv'),
                           key_col='MRID', target_col='DX',
                           data_cols=['DL_MUSE_Volume_*'],
                           preprocessing={'icv_correction': {
                               'icv_col': 'DL_MUSE_Volume_702',
                           }},
                           output_file=str(tmp_path / 'out.csv'))
